...
const AdminWalletRequests = () => {
  ...
  return (
    <div className="container mx-auto px-4 py-8">
      <h1 className="text-3xl font-bold text-foreground mb-6">Wallet Withdraw Requests</h1>
              ...
              <tbody>
                {withdrawRequests.map((req) => (
                  <tr key={req.id} className="border-t border-border hover:bg-muted/50 transition">
                    <td className="p-3">{req.profiles?.first_name} {req.profiles?.last_name}</td>
                    ...
                    <td className="p-3">{format(new Date(req.created_at), "MMM dd, yyyy")}</td>
                    <td className="p-3">
                      {req.status === "pending" && (
                        <Button
                          variant="success"
